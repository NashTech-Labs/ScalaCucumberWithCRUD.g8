...
class RouteSteps extends WordSpec with ScalaDsl with EN with MockitoSugar with Routes with  Matchers with ScalatestRouteTest {
  ...
  When("""^I hit on route post with value "([^"]*)""([^"]*)""([^"]*)"$"""){ (arg0:String, arg1:String, arg2:String) =>
    val path = arg0+ arg1 + arg2
    result = Post(s"/$path") ~> allRoute
  }

  When("""^I hit on route put with value "([^"]*)""([^"]*)""([^"]*)"$""") { (arg0: String, arg1: String, arg2: String) =>
    val path = arg0+ arg1 + arg2
    result = Put(s"/$path") ~> allRoute
  }

  When("""^I hit on route get with value "([^"]*)""([^"]*)""([^"]*)"$""") { (arg0: String, arg1: String, arg2: String) =>
    val path = arg0+ arg1 + arg2
    result = Get(s"/$path") ~> allRoute
  }
  When("""^I hit on route delete with value "([^"]*)""([^"]*)""([^"]*)"$""") { (arg0: String, arg1: String, arg2: String) =>
    val path = arg0+ arg1 + arg2
